...
using namespace std;
#define OnShFileName "onSh.txt"
#define CsMdFileName "csMd.txt"
void _pause();
...
void testOnShelf(){
  fstream onShfile(OnShFileName, ios::out);
  OnShelf onSh;
  bool ok = true;
  int i;
  ...
  if (ok){
    ...
    cout << "--Store OnShelf, program: " << endl
      << "N,1234,Box,123.45,0,1,5" << endl
      << "N,1234,Box,123.45,0,1,5" << endl;
    cout << "--Store OnShelf, yours: " << endl;
    dumpFile(OnShFileName);
    cout << "--Load OnShelf: " << endl;
    onShfile.open(OnShFileName, ios::in);
    onShfile.ignore(2);
    tonSh.load(onShfile);
...
void testCustomMade(){
  fstream csMdfile(CsMdFileName, ios::out);
  CustomMade csmd;
  bool ok = true;
  ...
  if (ok){
    ...
    cout << "--Store CustomMade, program: " << endl
      << "P,1234,centerpiece,123.45,1,1,15,2017/10/12" << endl
      << "P,1234,centerpiece,123.45,1,1,15,2017/10/12" << endl;
    cout << "--Store CustomMade, yours: " << endl;
    dumpFile(CsMdFileName);
    cout << "--Load CustomMade: " << endl;
    csMdfile.open(CsMdFileName, ios::in);
    csMdfile.ignore(2);
    tcsmd.load(csMdfile);
    cout << "Program: 1234   |centerpiece         | 139.50| t |   1|  15|2017/10/12" << endl;
...
void piv(const char* upc, const char* name, const char* price , const char* taxed, 
  const char* qty, const char* qtyNd, const char* date ){
  cout
